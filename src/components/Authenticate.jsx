        ...
        <Tab.Panels>
          <Tab.Panel>
            <form onSubmit={loginUser} className="flex flex-col mt-3">
              {error === "Incorrect username" && (
                <h1 className=" text-red-700 dark:text-red-400 font-bold">
                  {error}
                </h1>
              )}
              {error === "Missing credentials" && (
                <h1 className=" text-red-700 dark:text-red-400 font-bold mb-3">
                  Please enter a valid username and password
                </h1>
              )}
              <div
                className={`relative border p-2 mt-4 ${
                  error === "Incorrect username" ||
                  error === "Missing credentials"
                    ? "border-red-700"
                    : ""
              ...
              >
                <label
                  ...
                  className="absolute top-0 left-2 bg-white dark:bg-sky-950 dark:text-white px-1 text-xs md:text-base -translate-y-2/4"
                >
                  Username
                </label>
                <input
                  type="text"
                  className="w-full dark:bg-black dark:text-white rounded-2xl mt-2 p-2 focus:outline-none"
                  id="login-username"
              ...
              {error === "Incorrect password" && (
                <h1 className=" text-red-700 dark:text-red-400 mt-3 font-bold">
                  {error}
                </h1>
              )}
              <div
                className={`relative border p-2 mt-3 ${
                  error === "Incorrect password" ||
                  error === "Missing credentials"
                    ? "border-red-700"
                    : ""
              ...
              >
                ...
                <input
                  className="w-full dark:bg-black dark:text-white focus:outline-none  mt-2 rounded-2xl p-2"
                  type="password"
                  id="login-password"
              ...
              <input
                className="btn-submit  mt-4 w-full md:text-lg  border-2 bg-sky-600 text-white rounded-xl p-3 hover:bg-white hover:text-sky-900 hover:border-sky-900 ease-in-out duration-300"
                type="submit"
                value="Log In"
              />
            </form>
          ...
          <Tab.Panel>
            <form onSubmit={registerUser} className="flex flex-col mt-3">
              ...
              <div
                className={`relative border p-2 my-3 ${
                  error === "Username already exists" ||
                  error === "Missing credentials"
                    ? "border-red-700"
                    : ""
              ...
              >
                ...
                <input
                  type="text"
                  className="w-full dark:bg-black dark:text-white  mt-2 rounded-2xl p-2 focus:outline-none"
              ...
              <div
                className={`relative border p-2 mt-2 ${
